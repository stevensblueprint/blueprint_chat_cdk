...
export const handler = async (event: any) => {
  ...
  try {
    ...
    if (httpMethod === "POST" && path.match(/^\/conversations\/[^\/]+\/turns$/)) {
      ...
      const s3Object = await s3Client.send(new GetObjectCommand({ Bucket: BUCKET_NAME, Key: s3Key }));
    ...
    if (httpMethod === "DELETE" && path.match(/^\/conversations\/[^\/]+$/)) {
      ...
      return { statusCode: 200, body: JSON.stringify({ status: "deleted", deletedId: conversationId }) };
    }
